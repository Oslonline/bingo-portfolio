...
function ContactForm({ closeModal, updateSuccessMessage }) {
  ...
  const validateAndSendEmail = () => {
    ...
    emailjs
      ...
      .then(
        ...
        (error) => {
          console.log(
            "Mail failed to leave your computer. Something's wrong...",
            error.text
          );
  ...
  return (
    ...
    >
      {errorMessage && (
        <div className="w-fit rounded-lg bg-red-200 px-2 py-1 text-red-950">
          {errorMessage}
        </div>
      )}
      <div className="flex flex-col gap-1">
        <label
          className="select-none text-sm text-gray-700 dark:text-gray-300"
          htmlFor="username"
        >
          Full Name :
        </label>
        <input
          ...
          className="w-full rounded-lg border border-gray-500 bg-gray-50 p-2 focus:border-gray-900 focus:outline-hidden dark:border-transparent dark:bg-gray-700 dark:text-gray-50 dark:caret-white dark:focus:border dark:focus:border-gray-300"
          name="user_name"
          type="text"
          required
        />
      ...
      <button
        aria-label="Contact form validating button"
        type="button"
        onClick={validateAndSendEmail}
        className="flex items-center justify-center gap-3 rounded-lg bg-gray-900 px-4 py-2 text-gray-50 hover:bg-gray-700 dark:bg-gray-50 dark:text-gray-900 dark:hover:bg-gray-600 dark:hover:text-gray-50"
      >
        {isSubmitting ? (
          <>
            <svg
